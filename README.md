...
| 指标 | 数值 |
|---|---|
| 点/划判读(约 4,800 次敲击) | **98.6%** |
| 整字母解码(每次重复) | **97.7%** |
| 字母认对(多数投票) | **24 / 26**(含全部 7 个纯字母) |

- 仅 **C / X** 未认出——那两份录制本身**点划没做出区分**(点按太重、或差别太小时轻时重),重录即可,不是方法问题。
- 点/划判定是**逐字母组**做的:组内有高矮对比→按相对峰高;组内没对比(纯字母)→按绝对时长。早期版本(仅相对峰高、无去趋势)整字母 90.2%、纯字母无法判;加入**滚动去趋势 + 逐组『峰高/时长』判定**后提升到上表。
